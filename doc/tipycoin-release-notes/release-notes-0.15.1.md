Tipycoin Core version *0.15.1* is now available from:

  <https://download.tipycoin.org/tipycoin-0.15.1/>
...
A number of changes to the way Tipycoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 lites per kB (0.00001 TPC). This is to prepare for dropping the minimum transaction fee to 0.1 lites per kB (0.0001 TPC) in the next release.
